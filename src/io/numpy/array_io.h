...
/*!
 * Partial specialization for copying from a numpy buffer to a numpy array of strings
 * @tparam ArrayT
 */
...
class ArrayIO
{
    ...
    static bool canDeserialize(const Header& header, ArrayT& array)
    {
        ...
        {
            std::cerr
                << "Expected Number of dimensions does not match the dimensions of the numpy array."
                << std::endl;
            return false;
        }
        ...
        if (!header.dtype.match(expected_dtype))
        {
            std::cerr << "ArrayIO Error because of type mismatching" << std::endl
                      ...
                      << "File type is " << DATA_TYPE_TO_STR.at(header.dtype.type)
                      << header.dtype.bytes << std::endl;
            return false;
        }
...
public:
    ...
    {
        Reader reader;
        File::Ptr file_data = reader.read_file(numpy_file_path);
        if (file_data)
        {
